...
from MultipleInputEmbeddingWrapper import MultipleInputEmbeddingWrapper
...
def embedding_rnn_seq2seq(encoder_inputs, decoder_inputs, cell,
  ...
  with variable_scope.variable_scope(scope or "embedding_rnn_seq2seq"):
    # Encoder.
    encoder_cell = rnn_cell.EmbeddingWrapper2(
...
def embedding_attention_multiseq2seq(encoder_inputs, decoder_inputs, cell,
                                num_encoder_symbols, num_decoder_symbols,
                                embedding_size,
                                num_heads=1, output_projection=None,
                                feed_previous=False, dtype=dtypes.float32,
  ...
  with variable_scope.variable_scope(scope or "embedding_attention_seq2seq"):
    # Encoder.
    encoder_cell = MultipleInputEmbeddingWrapper(cell,
        embedding_classes=num_encoder_symbols, embedding_sizes=embedding_size)
    ...
    def decoder(feed_previous_bool):
      ...
      with variable_scope.variable_scope(variable_scope.get_variable_scope(),
        ...
        outputs, state = embedding_attention_decoder(
            ...
            initial_state_attention=initial_state_attention)
        return outputs + [state]
    print("embedding_attention_multiseq2seq:2")
    outputs_and_state = control_flow_ops.cond(feed_previous,
                                              lambda: decoder(True),
                                              lambda: decoder(False))
    return outputs_and_state[:-1], outputs_and_state[-1]
